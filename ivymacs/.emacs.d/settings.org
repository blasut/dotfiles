...
** persp-switch-by-number
   Switch to workspace by number (used for M-1 through M-9 bindings)
   #+BEGIN_SRC emacs-lisp
   (defun persp-switch-by-number (num)
     "Switch to workspace by number NUM."
     (let* ((persps (persp-names))
            (persp-name (nth (1- num) persps)))
       (when persp-name
...
* Workspaces
  Workspace management using persp-mode, inspired by Doom Emacs
** persp-mode
   #+BEGIN_SRC emacs-lisp
   (use-package persp-mode :ensure t
     ...
     ;; Create a main workspace instead of using the nil perspective
     (add-hook 'persp-mode-hook
               (lambda ()
                 (when persp-mode
                   (let ((main-persp (persp-get-by-name "main")))
                     (unless main-persp
                       (persp-add-new "main"))))))

     ;; Don't save TRAMP buffers or dead buffers
     ...
     ;; Integrate with projectile for per-project workspaces
     (with-eval-after-load 'projectile
       ;; Function to create/switch to workspace after project switch
       (defun my-persp-switch-project-hook ()
         "Create or switch to a workspace for the current project."
         (when (and persp-mode (projectile-project-p))
           (let* ((project-name (projectile-project-name))
                  (persp-name project-name))
             ;; Create or switch to workspace for this project
             (unless (string= (safe-persp-name (get-current-persp)) persp-name)
               (if (persp-get-by-name persp-name)
                   (persp-switch persp-name)
                 (persp-add-new persp-name))))))

       ;; Hook into projectile's after-switch-project hook
       (add-hook 'projectile-after-switch-project-hook #'my-persp-switch-project-hook)

       ;; Keep the default action as find-file
...
** general
   ...
   (use-package general :ensure t
     ...
     (general-define-key
      ...
      "<tab> r" '(persp-rename :which-key "Rename workspace")
      "TAB s" '(persp-save-state-to-file :which-key "Save workspace")
      "<tab> s" '(persp-save-state-to-file :which-key "Save workspace")
      "TAB l" '(persp-load-state-from-file :which-key "Load workspace")
      "<tab> l" '(persp-load-state-from-file :which-key "Load workspace")
      ...
      "<tab> b" '(persp-switch-to-buffer :which-key "Switch workspace buffer")
      "TAB k" '(persp-kill-buffer :which-key "Kill workspace buffer")
      "<tab> k" '(persp-kill-buffer :which-key "Kill workspace buffer"))
...
** workspace keybindings
   Additional convenient keybindings for workspaces
   #+BEGIN_SRC emacs-lisp
   (with-eval-after-load 'persp-mode
     ...
     ;; Quick numeric workspace switching (M-1 through M-9)
     (general-define-key
