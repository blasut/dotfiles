(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(custom-safe-themes
   (quote
    ("5dc0ae2d193460de979a463b907b4b2c6d2c9c4657b2e9e66b8898d2592e3de5" default)))
 '(package-selected-packages
   (quote
    (eval-in-repl exec-path-from-shell cider smooth-scrolling c-eldoc visual-regexp keyfreq restclient projectile hydra counsel swiper ace-window evil yasnippet flycheck parinfer evil-magit evil-surround slime htmlize org-babel-eval-in-repl which-key use-package rainbow-delimiters org-plus-contrib ob-restclient material-theme magit lispy general evil-escape dashboard counsel-projectile color-theme-solarized))))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(default ((t (:inherit nil :stipple nil :background "#FAFAFA" :foreground "#212121" :inverse-video nil :box nil :strike-through nil :overline nil :underline nil :slant normal :weight normal :height 160 :width normal :foundry "nil" :family "Office Code Pro"))))
 '(rainbow-delimiters-depth-5-face ((t (:foreground "#26A69A"))))
 '(rainbow-delimiters-depth-6-face ((t (:foreground "#B71C1C"))))
 '(rainbow-delimiters-depth-7-face ((t (:foreground "#795547")))))
